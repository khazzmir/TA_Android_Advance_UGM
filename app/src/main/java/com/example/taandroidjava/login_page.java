package com.example.taandroidjava;

import androidx.annotation.NonNull;
import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.view.WindowManager;
import android.widget.Toast;

import com.google.android.material.textfield.TextInputEditText;
import com.google.firebase.database.DataSnapshot;
import com.google.firebase.database.DatabaseError;
import com.google.firebase.database.DatabaseReference;
import com.google.firebase.database.FirebaseDatabase;
import com.google.firebase.database.Query;
import com.google.firebase.database.ValueEventListener;

import java.util.Objects;

public class login_page extends AppCompatActivity {

    DatabaseReference databaseReference = FirebaseDatabase.getInstance("https://ta-android-java-default-rtdb.asia-southeast1.firebasedatabase.app/").getReferenceFromUrl("https://ta-android-java-default-rtdb.asia-southeast1.firebasedatabase.app/");

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        setContentView(R.layout.activity_login_page);
    }


    public void db(){

        final TextInputEditText fullname = findViewById(R.id.fullname);
        final TextInputEditText password = findViewById(R.id.password);

        final String fullnametxt = fullname.getText().toString().trim();
        final String passwordtxt = password.getText().toString().trim();

        if(fullnametxt.isEmpty() || passwordtxt.isEmpty()){
            Toast.makeText(this, "Masukan Nama Lengkap dan Password Anda", Toast.LENGTH_SHORT).show();
        }else {
            databaseReference.child("Users").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {

                    if(snapshot.hasChild(fullnametxt)) {
                        final String getpassword = snapshot.child(fullnametxt).child("password").getValue(String.class);

                        if (getpassword.equals(passwordtxt)) {
                            Toast.makeText(login_page.this, "Login Berhasil", Toast.LENGTH_SHORT).show();

                            startActivity(new Intent(login_page.this, MainActivity.class));
                            finish();
                        } else {
                            Toast.makeText(login_page.this, "Password Salah", Toast.LENGTH_SHORT).show();
                            password.requestFocus();
                        }
                    } else {
                        Toast.makeText(login_page.this, "User Tidak Tersedia", Toast.LENGTH_SHORT).show();
                    }
                }

                @Override
                public void onCancelled(@NonNull DatabaseError error) {

                }
            });
        }
    }

    public void check(){
        final TextInputEditText fullname = findViewById(R.id.fullname);
        final TextInputEditText password = findViewById(R.id.password);

        final String fullnametxt = fullname.getText().toString().trim();
        final String passwordtxt = password.getText().toString().trim();

        DatabaseReference reference = databaseReference;
        Query check = reference.orderByChild("fullname").equalTo(fullnametxt);

        check.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    fullname.setError(null);
                    String pw = snapshot.child(fullnametxt).child("password").getValue(String.class);

                    if(!Objects.equals(pw, passwordtxt)){
                        Intent intent = new Intent(login_page.this, MainActivity.class);
                        startActivity(intent);
                        databaseReference.child("Users").child(fullnametxt).child("login").setValue("yes");
                        finish();
                    }else{
                        Toast.makeText(login_page.this, "Password Salah", Toast.LENGTH_SHORT).show();
                        password.requestFocus();
                    }
                }else{
                    Toast.makeText(login_page.this, "User Tidak Tersedia", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {

            }
        });

    }

    public void sg(View view) {
        Intent intent = new Intent(this, signup_page.class);
        startActivity(intent);
        finish();
    }

    public void login(View view) {
        db();
    }
}